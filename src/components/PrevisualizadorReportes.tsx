...
import { Eye, Download, Loader2, Table2, X, Check, FileText, Link2, ChevronDown, ChevronUp } from "@/components/icons";
...
function cellClass(label: string, val: string) {
  ...
  if (val === "✓ OK" || val === "activo" || val === "aprobada" || val === "vigente" || val === "entregada") return "text-brand-green font-medium";
  if (val === "pendiente") return "text-yellow-300 font-medium";
  return "";
}

export default function PrevisualizadorReportes() {
  ...
  return (
    <div className="space-y-4">
            ...
            return (
              ...
              >
                ...
                <div className="min-w-0">
                  ...
                  <p className="text-[11px] text-text-soft/55 mt-0.5 leading-tight">{info.descripcion}</p>
                </div>
              </button>
            );
          })}
      ...
      <div className="rounded-xl border border-white/10 bg-white/3 p-4 space-y-3">
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            ...
            >
              {colsCollapsed ? <ChevronDown className="h-3.5 w-3.5" /> : <ChevronUp className="h-3.5 w-3.5" />}
            </button>
          </div>
        </div>
      ...
      >
        ...
        {!preview && !loading && (
          <div
            className="flex flex-col items-center justify-center flex-1 text-center text-text-soft"
            style={{ minHeight: "460px" }}
          >
          ...
          <>
            ...
            <div className="flex items-center justify-between px-4 py-2.5 bg-white/5 border-b border-white/10 flex-shrink-0">
              ...
              <button
                ...
                className="flex-shrink-0 rounded p-1 text-text-soft hover:text-white hover:bg-white/10 transition-colors"
              >
            ...
            {/* Barra inferior */}
            <div className="flex items-center justify-between px-4 py-2 bg-white/3 border-t border-white/10 flex-shrink-0">
              <p className="text-[11px] text-text-soft/60">
                Vista previa:{" "}
                <strong className="text-white/70">{preview.filas.length}</strong> filas ·
                La descarga incluye <strong className="text-white/70">todos</strong> los registros
              </p>
              <div className="flex items-center gap-3">
